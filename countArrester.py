...
def countArrester(image,info):
    temple=cv2.imread("template/"+info["template"])
    meter = meterFinderByTemplate(image, temple)
    image = meter
    ...
    for c in redContours:
        for p in c:
            ...
            if dis > maxDis:
                maxDis = dis
                pointerPoint = p[0]
    print(startPoint, startPoint, centerPoint,pointerPoint)
    #value=AngleFactory.calPointerValueByOuterPoint(np.array(startPoint), np.array(startPoint), centerPoint,
     #                                        pointerPoint, 0, info["totalValue"])
